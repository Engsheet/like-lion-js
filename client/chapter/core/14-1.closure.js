...
//* 1. 전역의 오염을 막기 위해
//* 2. 값을 기억하기 위해 (가비지 컬렉터 수집 X)
//* 3. 함부로 내가 설정한 값을 수정할 수 없게 보호 (특정 함수를 통해서만 접근할 수 있도록)
...
// result를 호출하면 함수 본문이 호출되기 때문에 함수로 인식됨
// result를 실행할 때마다 값이 1씩 증가하는 함수가 생성됨
let result = counter();
...
const ufo = earth()
ufo(5)
// 클로저 : 태어난 환경을 기억하는 호랑이

/* -------------------------------------------------------------------------- */

const button = document.querySelector('button')

// let isClicked = true; // 전역 오염.. 아래쪽에서 재정의하게되면 코드가 망가짐
